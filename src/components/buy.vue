    ...
    <v-carousel show-arrows="hover" height="210" >
      ...
      ></v-carousel-item>
           ...
           <v-col cols="8">
            ...
            <h4 @click="gotoProduct()"> {{item.propertyName}} </h4>
            <p @click="gotoProduct()">{{item.propertyDescription}}</p>
            <!--location -->
            <div class="ma-3">  <v-icon>mdi-map-marker</v-icon> {{` ${item.propertyLocation.province} ${ item.propertyLocation.ward} ${item.propertyLocation.street} `}}</div>
            <!-- price & Availability --> 
